...
export class CloudFrontBuilder {
  ...
  setAliasConfiguration({ acmCertRef, names }: CloudFrontAliasConfigurationProps): CloudFrontBuilder {
  ...
  setLogginConfiguration({ logBucket, logPrefix }: CloudFrontLogginConfiguarionProps): CloudFrontBuilder {
  ...
  build(scope: Construct, id: string): CloudFrontWebDistribution {
    ...
    return new CloudFrontWebDistribution(scope, `${id}-Distribution`, this.cloudfrontWebDistributionProps)
  }

  buildOriginAccessIdentity(scope: Construct, comment?: string): OriginAccessIdentity {
